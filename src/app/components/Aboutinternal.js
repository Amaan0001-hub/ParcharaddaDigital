...
const Aboutbanner = () => {
  ...
  const getPageTitle = () => {
    ...
    // For dynamic routes like /team-details/[id]
    if (pathname?.startsWith('/team-details/')) {
      return 'Team Details';
    }
  ...
  const getBreadcrumbs = () => {
    ...
    paths.forEach((path, index) => {
      ...
      // Check if this is a numeric ID (dynamic route parameter)
      const isNumericId = !isNaN(path) && path.trim() !== '';
      ...
      // For /team-details path
      if (currentPath === '/team-details') {
        name = 'Team Details';
      }
      // For /team-details/[id] (parent path)
      else if (currentPath.startsWith('/team-details/') && !isNumericId) {
        name = 'Team Details';
      }
      else if (pageTitles[currentPath]) {
        name = pageTitles[currentPath];
      } else {
        name = path.split('-')
          .map(word => word.charAt(0).toUpperCase() + word.slice(1))
          .join(' ');
